...
class Products extends CI_Controller {
    ...
    public function getProducts(){
        ...
        $products = $this->db->query("
            select
                p.*,
                concat(p.Product_Name, ' - ', p.Product_Code, ' - ', p.body_number) as display_text,
                pc.ProductCategory_Name,
                br.brand_name,
                u.Unit_Name,
                p.Product_Purchase_Rate as averagePurchasePrice
            from tbl_product p
    ...
    public function getProductStock(){
        ...
        echo $this->mt->productStock($inputs->productId);
    }
        ...
        $stock = $this->db->query("
            select * from(
                select
                    ...
                    round(ifnull((
                        select 
                            ifnull(sum(pd.PurchaseDetails_TotalQuantity * pd.PurchaseDetails_Rate), 0) / 
                            ifnull(sum(pd.PurchaseDetails_TotalQuantity), 0) 
                        from tbl_purchasedetails pd 
                        where pd.Product_IDNo = p.Product_SlNo
                        and pd.Status = 'a'
                    ), 0), 2) as average_purchase_rate,
