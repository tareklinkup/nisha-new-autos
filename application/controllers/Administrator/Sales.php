        ...
        try {
            ...
            foreach ($data->cart as $cartProduct) {
                $saleDetails = array(
                    ...
                    'averagePurchasePrice' => $cartProduct->averagePurchasePrice,
                ...
                //update product sales rate
                if ($data->sales->salesType == 'retail') {
                    $this->db->query("
                        update tbl_product set
                        Product_SellingPrice = ?,
                        body_rate = ?
                        where Product_SlNo = ?
                    ", [$cartProduct->salesRate, $cartProduct->bodyRate, $cartProduct->productId]);
                } else {
                    $this->db->query("update tbl_product set Product_WholesaleRate = ? where Product_SlNo = ?", [$cartProduct->salesRate, $cartProduct->productId]);
                }
            }
...
<?php
        ...
        foreach ($sales as $sale) {
            $sale->saleDetails = $this->db->query("
                select
                    ...
                    p.Product_Code,
                    p.Product_Name,
                    (sd.averagePurchasePrice * sd.SaleDetails_TotalQuantity) as purchased_amount,
    ...
    public function get_product_profit()
    {
        ...
        if ($data->products != null && $data->products != '') {
            $customerClause = " and sd.Product_IDNo = '$data->products'";
        }
        if ($data->brand != null && $data->brand != '') {
            $customerClause = " and b.brand_SiNo  = '$data->brand'";
        }
        $dateClause = "";
        if (($data->dateFrom != null && $data->dateFrom != '') && ($data->dateTo != null && $data->dateTo != '')) {
            $dateClause = " and sm.SaleMaster_SaleDate between '$data->dateFrom' and '$data->dateTo'";
        ...
        $product_profit = $this->db->query("
            ...
            join tbl_product p on p.Product_SlNo = sd.Product_IDNo
            left JOIN tbl_brand b on b.brand_SiNo = p.brand 
            join tbl_salesmaster sm on sd.SaleMaster_IDNo = sm.SaleMaster_SlNo
            where sd.Status = 'a'  
            $customerClause     
            $dateClause 
        ")->result();
        echo json_encode($product_profit);
    }
