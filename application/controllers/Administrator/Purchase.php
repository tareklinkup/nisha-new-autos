...
class Purchase extends CI_Controller
{
    ...
    public function addPurchase()
    {
        ...
        try{
            ...
            foreach($data->cartProducts as $product){
                ...
                $purchaseDetails = array(
                    ...
                    'Purchasedetails_avg_price' => $product->averagePurchasePrice,
                    'Status' => 'a',
                    'AddBy' => $this->session->userdata("FullName"),
                    'AddTime' => date('Y-m-d H:i:s'),
                ...
                ", [
                    $previousStock,
                    $product->total,
                    ($product->quantity + $previousStock),
                    $product->bodyRate, 
                    $product->productId
                ]);
            }
        ...
        $xx['purchaseforprint'] = $purchaseId;
    ...
    public function purchase_bill()
    {
        ...
        $data['invoices'] = $this->Purchase_model->get_purchase_invoice_info();
    ...
    public function purchase_to_report()
    {
        ...
        $sql = $this->db->query("SELECT * FROM tbl_purchasemaster WHERE PurchaseMaster_SlNo = '$id'");
        $row = $sql->row();
        $datas['PurchID'] = $row->PurchaseMaster_SlNo;
        $datas['invoices'] = $row->PurchaseMaster_InvoiceNo;
        $this->session->set_userdata($datas);
        $data['content'] = $this->load->view('Administrator/purchase/purchase_to_report', $datas, TRUE);
        $this->load->view('Administrator/index', $data);
    }
