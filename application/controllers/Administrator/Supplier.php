...
class Supplier extends CI_Controller {
    ...
    function supplier_payment_histry(){
        $access = $this->mt->userAccess();
        if(!$access){
            redirect(base_url());
        }
        $data['title'] = "Supplier Payment Histry";
        $data['content'] = $this->load->view('Administrator/payment_reports/supplier_payment_histry', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function getSupplierPayments(){
        ...
        if(isset($data->supplierId) && $data->supplierId != ''){
            $dateClause = " and sp.SPayment_customerID = '$data->supplierId'";
        }
