						...
						<div class="col-sm-6">
							<form v-on:submit.prevent="addToCart">
								...
								<div class="form-group">
									...
									<div class="col-md-8">
										<input type="text" class="form-control" v-model="selectedProduct.brand_name" readonly>
									</div>
								</div>
								<div class="form-group">
									<label class="col-md-4">Body Rate</label>
									<div class="col-md-3">
										<input type="number" class="form-control" required v-model="selectedProduct.body_rate">
									</div>
									<label class="col-md-2">Quantity</label>
									<div class="col-md-3">
										<input type="number" class="form-control" required v-model="selectedProduct.quantity" v-on:input="productTotal" ref="quantity">
									</div>
								</div>
								<div class="form-group">
									<label class="col-md-4">Purchase In%</label>
									<div class="col-md-3">
										<input type="number" step="0.01" min="0.00" class="form-control" required v-model="selectedProduct.PurchaseIn" @input="calculatePrice">
									</div>
									<label class="col-md-2" style="padding-right: 0;text-align: right;">P. Price</label>
									<div class="col-md-3">
										<input type="number" class="form-control" required v-model="selectedProduct.Product_Purchase_Rate" @input="calculatePRate">
									</div>
								</div>

								<!-- <div class="form-group">
									<label class="col-sm-3 control-label no-padding-right"> Purchase In % </label>
									<div class="col-sm-3">
										<input type="text" placeholder="Sale In" class="form-control" v-model="selectedProduct.saleIn" @input="calculateSalePrice" required />
									</div>
									<label class="col-sm-3 control-label no-padding-right"> Pur. Price </label>
									<div class="col-sm-3">
										<input type="number" placeholder="Rate" step="0.01" class="form-control" v-model="selectedProduct.Product_SellingPrice" @input="calculateSaleIn" required />
									</div>
								</div> -->

								<div class="form-group">
									<!-- <label class="col-md-4">Purchase Rate</label>
									<div class="col-md-3">
										<input type="number" class="form-control" required v-model="selectedProduct.Product_Purchase_Rate" v-on:input="productTotal">
									</div> -->
									<label class="col-md-4">Total</label>
									<div class="col-md-8">
										<input type="number" class="form-control" readonly v-model="selectedProduct.total">
									</div>
								</div>
				...
				<table class="table table-bordered table-condensed">
					...
					<tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
						<tr v-for="(product, sl) in cart">
							...
							<td>{{ product.bodyNumber }}</td>
							<td>{{ product.name }}</td>
							<td>{{ product.bodyRate }}</td>
							<td><input type="number" v-model="product.purchaseRate" v-on:input="calculateTotal" style="width:100px;"></td>
							<td><input type="number" v-model="product.quantity" v-on:input="calculateTotal" style="width:100px;"></td>
							<td>{{ product.total }}</td>
						...
						<tr>
							<td colspan="5"><textarea class="form-control" v-model="purchase.description"></textarea></td>
							<td colspan="4">{{ purchase.subTotal }}</td>
						</tr>
					</tbody>
				</table>
			</div>
...
<script>
	...
	new Vue({
		...
		methods: {
			...
			calculatePrice() {
				this.selectedProduct.Product_Purchase_Rate = (this.selectedProduct.body_rate * this.selectedProduct.PurchaseIn).toFixed(2);
				this.productTotal();
			},
			calculatePRate() {
				this.selectedProduct.PurchaseIn = (this.selectedProduct.Product_Purchase_Rate / this.selectedProduct.body_rate).toFixed(2);
				this.productTotal();
			},
			getProducts() {
			...
			addToCart() {
				...
				let product = {
					...
					bodyNumber: this.selectedProduct.body_number,
					brandName: this.selectedProduct.brand_name,
					bodyRate: this.selectedProduct.body_rate,
					purchaseRate: this.selectedProduct.Product_Purchase_Rate,
					averagePurchasePrice: this.selectedProduct.averagePurchasePrice,
			...
			calculateTotal() {
				this.cart.forEach(product => {
					product.total = product.quantity * product.purchaseRate;
				})
			...
			savePurchase() {
				...
				axios.post(url, data).then(async res => {
					...
					if (r.success) {
						...
						if (conf) {
							window.open('/purchaseToReport', '_blank');
				...
				axios.post('/get_purchases', {
					...
					r.purchaseDetails.forEach(product => {
						let cartProduct = {
							...
							bodyNumber: product.body_number,
							brandName: product.brand_name,
							bodyRate: product.body_rate,
							purchaseRate: product.PurchaseDetails_Rate,
							averagePurchasePrice: product.Purchasedetails_avg_price,
