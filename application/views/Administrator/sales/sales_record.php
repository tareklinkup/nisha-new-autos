					...
					<tbody>
						<template v-for="sale in sales">
							<tr>
								...
								<td>{{ sale.AddBy }}</td>
								<td>{{ sale.saleDetails.Product_Name }}</td>
								<td style="text-align:right;">{{ sale.saleDetails.SaleDetails_Rate }}</td>
								<td style="text-align:center;">{{ sale.saleDetails.SaleDetails_TotalQuantity }}</td>
								<td style="text-align:right;">{{ sale.saleDetails.SaleDetails_TotalAmount }}</td>
				...
				<table 
					...
					<tfoot>
						<tr style="font-weight:bold;">
							...
							<td style="text-align:right;">{{ sales.reduce((prev, curr) => { return prev + parseFloat(curr.SaleDetails_TotalQuantity)}, 0) | decimal }}</td>
						</tr>
					</tfoot>
				</table>
			</div>
