		...
		<div class="row" style="padding-top:10px;">
			...
			<div class="col-md-6">
				...
				<table class="table table-bordered table-condensed">
					...
					<tbody style="display:none;" v-bind:style="{display: vatPaid.length > 0 ? '' : 'none'}">
						<tr v-for="transaction in vatPaid">
							<td>{{ transaction.Tr_Id }}</td>
							<td>{{ transaction.Tr_date }}</td>
							<td>{{ transaction.Acc_Name }}</td>
							<td style="text-align:right;">{{ transaction.Out_Amount | decimal }}</td>
					...
					<tfoot>
						<tr style="font-weight:bold;">
							<td colspan="3" style="text-align:right">Total</td>
							<td style="text-align:right;">
								<span v-if="vatPaid.length == 0">0.00</span>
								<span style="display:none;" v-bind:style="{display: vatPaid.length > 0 ? '' : 'none'}">{{ totalVatPaid | decimal }}</span>
							</td>
						</tr>
					</tfoot>
				</table>
				...
				<!-- Employee Old Payments -->
				<table class="table table-bordered table-condensed">
					<thead>
						<tr style="background: #dee4dc;">
							<th colspan="5">Employee Old Payments</th>
						</tr>
						<tr>
							<th>Employee Id</th>
							<th>Employee Name</th>
					...
					<tbody style="display:none;" v-bind:style="{display: employeeOldPayments.length > 0 ? '' : 'none'}">
					...
					<tfoot>
						<tr style="font-weight:bold;">
							<td colspan="4" style="text-align:right">Total</td>
							<td style="text-align:right;">
								<span v-if="employeeOldPayments.length == 0">0.00</span>
								<span style="display:none;" v-bind:style="{display: employeeOldPayments.length > 0 ? '' : 'none'}">{{ totalEmployeeOldPayments | decimal }}</span>
							</td>
						</tr>
					</tfoot>
				</table>
...
<script>
	new Vue({
		...
		computed: {
			...
			totalVatPaid() {
				return this.vatPaid.reduce((prev, curr) => {
					return prev + parseFloat(curr.Out_Amount)
				}, 0).toFixed(2);
			},
			totalBankDeposit() {
		...
		methods: {
			...
			getVatPaid() {
				let filter = {
					dateFrom: this.filter.dateFrom,
					dateTo: this.filter.dateTo,
					transactionType: 'vat_paid'
				}
				axios.post('/get_cash_transactions', filter)
					.then(res => {
						this.vatPaid = res.data;
					})
			},
			...
			getEmployeeOldPayments(){
				...
				.then(res => { 
					this.employeeOldPayments = res.data;
				})
			},
